...
export function LearningWorkspace({ content, mdxContent }: { content: VideoContent; mdxContent?: ReactNode }) {
  ...
  const introCopy = video.introDescription ?? video.subtitle ?? `${Math.ceil((video.durationSeconds ?? 0) / 60)}분 안에 ${video.title}을 이해합니다.`;
  ...
  return <main>
    ...
    <article id="top" className="learning-page">
      <section className="intro-grid"><div className="hero-copy"><p className="context-label">오늘의 바이브코딩 학습</p><h1>{video.title}</h1><p className="value-copy">{introCopy}</p><div className="learning-stats" aria-label="학습 시간"><span><strong>{Math.ceil((video.durationSeconds ?? 0) / 60)}분</strong>전체 영상</span><span><strong>{Math.ceil(requiredSeconds / 60)}분</strong>필수 구간</span><span><strong>{STEP_COUNT}단계</strong>학습 과정</span></div><div className="hero-actions"><button onClick={() => moveToSegment(segments[0]?.startSeconds ?? 0)}>필수 구간부터 보기</button><a href="#summary">3줄 핵심 먼저 읽기</a></div></div>
        <aside className="route-card"><span>오늘의 학습 순서</span><ol>{routeCardSteps.map((step) => <li key={step.title}><b>{step.title}</b><small>{step.small}</small></li>)}</ol></aside></section>
      ...
      <div className="workspace-grid"><div className="main-flow">
        ...
        <section id="segments" className="step-section"><div className="step-heading"><span>2</span><div><p>{segmentsSectionTitle}</p><h2>필요한 구간만 바로 보기</h2></div></div><div className="segment-list">{segments.map((segment) => <button className="segment-card" key={`${segment.startSeconds}-${segment.endSeconds}`} onClick={() => moveToSegment(segment.startSeconds)}><span className={`segment-label ${segment.type}`}>{labels[segment.type]}</span><span className="segment-time">{formatTime(segment.startSeconds)}–{formatTime(segment.endSeconds)}</span><strong>{segment.title}</strong><small>{segment.reason}</small><i>이 구간 보기 →</i></button>)}</div></section>
        <section id="practice" className="step-section"><div className="step-heading"><span>3</span><div><p>{practiceSectionTitle}</p><h2>안전한 환경을 직접 설계하기</h2></div></div><ol className="practice-list">{practiceSteps.map((step, index) => <li key={step.title}><span>{index + 1}</span><div><strong>{step.title}</strong><p>{step.instruction}</p></div></li>)}</ol>{copyBlocks[0] && <div className="practice-tool"><p>실습에 바로 쓰는 프롬프트</p><CopyBlock title={copyBlocks[0].title} value={copyBlocks[0].content} language={copyBlocks[0].language} /></div>}</section>
