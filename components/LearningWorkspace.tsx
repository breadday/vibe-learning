...
export function LearningWorkspace({ content, mdxContent }: { content: VideoContent; mdxContent?: ReactNode }) {
  ...
  const introCopy = video.introDescription ?? video.subtitle ?? `${Math.ceil((video.durationSeconds ?? 0) / 60)}분 안에 ${video.title} 이해하기`;
  ...
  return <main>
    ...
    <article id="top" className="learning-page">
      <section className="intro-grid"><div className="hero-copy"><p className="context-label">{contextLabel}</p><h1>{video.title}</h1><p className="value-copy">{introCopy}</p><div className="learning-stats" aria-label="학습 시간"><span><strong>{Math.ceil((video.durationSeconds ?? 0) / 60)}분</strong>전체 영상</span><span><strong>{Math.ceil(requiredSeconds / 60)}분</strong>필수 구간</span><span><strong>{STEP_COUNT}단계</strong>학습 과정</span></div><div className="hero-actions"><button onClick={() => moveToSegment(segments[0]?.startSeconds ?? 0)}>{heroPrimaryCta}</button><a href="#summary">{heroSecondaryCta}</a></div></div>
